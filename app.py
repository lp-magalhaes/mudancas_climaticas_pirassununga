...
# Carregar os modelos treinados com tratamento de erros simplestry:
    model_rf = joblib.load('modelo_rf_vazao.pkl')except FileNotFoundError:
    st.error("❌ Erro: O arquivo 'modelo_rf_vazao.pkl' não foi encontrado no repositório.")
    st.stop()
try:
    model_xgb = joblib.load('best_xgboost_model.pkl')except FileNotFoundError:
    st.error("❌ Erro: O arquivo 'best_xgboost_model.pkl' não foi encontrado no repositório.")
    st.stop()
# 2. DADOS HISTÓRICOS REAIS E VALORES DE TURBIDEZ BASE INTERPOLADOSdados_base = {
...
st.sidebar.header("🎛️ Cenário de Aquecimento Global")delta_temp = st.sidebar.slider("Variação da Temperatura Média (°C)", min_value=-5.0, max_value=5.0, value=0.0, step=0.1)
# Gatilho Climático Regional (-7% de pluviosidade por grau de aquecimento)queda_chuva_por_grau = -0.07 delta_chuva_percentual = delta_temp * queda_chuva_por_grau * 100
# 4. PROCESSAMENTO HIDROLÓGICO DO CENÁRIOdf_sim = df_base.copy()
...
# Cálculo auxiliar do Balanço de Solo (Gráfico de Excedente)I = np.sum((df_sim['Temp_Sim'] / 5.0) ** 1.514)a = (6.75e-7 * I**3) - (7.71e-5 * I**2) + (1.792e-2 * I) + 0.49239
...
# =====================================================================# 5. EXECUÇÃO DO MODELO RANDOM FOREST (PREDIÇÃO DA VAZÃO)# =====================================================================vazao_base = []vazao_sim = []
for i in range(12):
...
# =====================================================================# 6. CÁLCULO DIRETO DAS CHUVAS ACUMULADAS (45 E 60 DIAS)# =====================================================================chuva_45_base, chuva_45_sim = [], []chuva_60_base, chuva_60_sim = [], []
c_base = list(df_sim['Chuva_Base'].values)c_sim = list(df_sim['Chuva_Sim'].values)
for i in range(12):
...
# =====================================================================# 7. EXECUÇÃO DO MODELO XGBOOST (PREDIÇÃO DA TURBIDEZ)# =====================================================================turb_sim = []for i in range(12):
...
# =====================================================================# 8. CÁLCULO DO CUSTO DO TRATAMENTO DE ÁGUA MÊS A MÊS# =====================================================================FATOR_SENSIBILIDADE_CUSTO = 0.1162
custos_incremento_mensal = []for i in range(12):
...
# =====================================================================# 9. EXIBIÇÃO DOS INDICADORES DE TOPO# =====================================================================col1, col2, col3, col4 = st.columns(4)
col1.metric("🌡️ Aquecimento Médio", f"{delta_temp} °C")
col2.metric("📉 Alteração Chuva (IPCC)", f"{delta_chuva_percentual:.1f} %")
v_sim_ago = float(df_sim['Vazao_Sim'].iloc[7])v_base_ago = float(df_sim['Vazao_Base'].iloc[7])queda_vazao_ago = ((v_sim_ago - v_base_ago) / v_base_ago) * 100
...
st.markdown("### 📊 Comportamento Sazonal da Vazão e Excedente Hídrico")fig1, ax1 = plt.subplots(figsize=(11, 3.5))ax2 = ax1.twinx()
...
st.markdown("### 📈 Diagnóstico de Qualidade da Água e Impacto Financeiro")col_graph1, col_grid2 = st.columns(2)
with col_graph1:
...
st.markdown("### 📝 Matriz de Variáveis Hidrológicas e Econômicas")df_exibicao = df_sim[['Mês', 'Chuva_Sim', 'Temp_Sim', 'Vazao_Sim', 'Turb_Base', 'Turb_Sim', 'Aumento_Custo_Pct']].copy()
